...
export class UsersService {
  ...
  getUsers(): iUsers[] {
    return this.usersArr.map((user) => ({
      ...
      image: user.image,
      title: user.title,
      toDo: Array.isArray(user.toDo) ? user.toDo : [], // Assicurati che toDo sia un array
    })) as iUsers[];
  }
  ...
  getUserWithToDo(todoList: iTodo[]): iUsers[] {
    return this.usersArr.map((user) => {
      // Trova il primo toDo associato all'utente
      const foundToDo = todoList.filter((toDo) => user.id === toDo.userId);

      // Se non viene trovato alcun toDo, restituisci l'utente senza modificarlo
      if (foundToDo.length === 0) return { ...user, toDo: [] };

      // Altrimenti, restituisci l'utente con il suo array di toDo
      return {
        ...user,
        toDo: foundToDo, // foundToDo è già un array
      };
    });
  }
